  ...
  module Tags
    class LatexBlock < Liquid::Block
      ...
      @@globals = {
        ...
        "resize" => "50%",
        "usepackages" => "",
        "latex_cmd" => "latex -interaction=nonstopmode $texfile",
        "dvips_cmd" => "dvips -E $dvifile -o $epsfile",
        "convert_cmd" => "convert -density $density $epsfile $pngfile",
        "scale_cmd" => "convert $pngfile -resize $resize $pngfile",
      ...
      def execute_cmd(cmd)
        ...
        cmd = cmd.gsub("\$resize", @p["resize"].to_s)
      ...
      def render(context)
        ...
        if !File.exists?(@p["png_fn"])
          ...
          execute_cmd(@@globals["scale_cmd"]) if ok
